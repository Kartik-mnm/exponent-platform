    ...
    <>
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,padding:"0 5%",height:64,display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled||mobileOpen?`${C.bg}ee`:"transparent",backdropFilter:scrolled||mobileOpen?"blur(20px)":"none",borderBottom:scrolled||mobileOpen?`1px solid ${C.border}`:"1px solid transparent",transition:"all 0.3s"}}>
        ...
        <div style={{display:"flex",gap:28,alignItems:"center"}}>
          {navLinks.map(l => (
            <button key={l.id} onClick={() => scrollTo(l.id)} style={{fontSize:13,color:C.t2,background:"none",border:"none",cursor:"pointer",fontWeight:500,padding:0,display:"var(--nav-link-display, inline-block)"}}>{l.label}</button>
          ))}
        </div>
      ...
      {mobileOpen && (
        <div style={{position:"fixed",top:64,left:0,right:0,zIndex:99,background:`${C.bg}f5`,backdropFilter:"blur(20px)",borderBottom:`1px solid ${C.border}`,padding:"20px 5% 24px",display:"flex",flexDirection:"column",gap:4}}>
          {navLinks.map(l => (<button key={l.id} onClick={()=>handleLink(l.id)} style={{fontSize:15,color:C.t1,background:"none",border:"none",cursor:"pointer",fontWeight:600,padding:"12px 0",textAlign:"left",borderBottom:`1px solid ${C.border}`}}>{l.label}</button>))}
          <div style={{display:"flex",gap:10,marginTop:16,flexDirection:"column"}}>
            <button onClick={()=>{setMobileOpen(false);onLogin();}} style={{padding:"11px",borderRadius:10,background:C.bg3,border:`1px solid ${C.border2}`,color:C.t1,fontWeight:700,fontSize:14,cursor:"pointer"}}>Sign In</button>
            <button onClick={()=>{setMobileOpen(false);onGetStarted();}} style={{padding:"11px",borderRadius:10,background:`linear-gradient(135deg,${C.acc},${C.pur})`,border:"none",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",boxShadow:`0 4px 16px ${C.acc}44`}}>🚀 Get Started Free →</button>
          </div>
        </div>
      )}
      <style>{`@media(max-width:768px){.hamburger-btn{display:block!important}:root{--nav-link-display:none}}@media(min-width:769px){.hamburger-btn{display:none!important}:root{--nav-link-display:inline-block}}`}</style>
    </>
  );
}
  ...
  return (
    <section style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",textAlign:"center",padding:"120px 5% 80px",background:`radial-gradient(ellipse 80% 60% at 50% -20%,${C.acc}18 0%,transparent 70%)`,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:800,margin:"0 auto"}}>
        <div style={{marginBottom:28}}><span style={{display:"inline-flex",alignItems:"center",gap:8,padding:"6px 16px",borderRadius:20,background:`${C.grn}14`,border:`1px solid ${C.grn}33`,fontSize:12,color:C.grn,fontWeight:600}}>✨ Trusted by coaching academies across India</span></div>
        <h1 style={{fontSize:"clamp(36px,6vw,64px)",fontWeight:900,lineHeight:1.1,letterSpacing:"-1.5px",color:C.t1,marginBottom:20}}>Run Your Academy<br /><span style={{background:`linear-gradient(135deg,${C.acc2},${C.pur})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Like a Pro — Not a Spreadsheet.</span></h1>
        ...
        <div style={{display:"flex",minHeight:360}}>
          ...
          <div style={{flex:1,padding:20,overflow:"hidden"}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
              {stats.map(s=>(<div key={s.l} style={{background:C.bg3,borderRadius:10,padding:"12px 14px",border:`1px solid ${C.border}`,borderTop:`2px solid ${s.c}`}}><div style={{fontSize:9,color:C.t3,fontWeight:600,marginBottom:6,textTransform:"uppercase"}}>{s.l}</div><div style={{fontSize:20,fontWeight:800,color:s.c}}>{s.i} {s.v}</div></div>))}
            </div>
  ...
  return (
    <section style={{padding:"40px 5%",borderTop:`1px solid ${C.border}`,borderBottom:`1px solid ${C.border}`}}>
      ...
      <div style={{display:"flex",gap:40,justifyContent:"center",flexWrap:"wrap",alignItems:"center"}}>
        {["Nishchay Academy","BrightFuture Coaching","TargetIIT Classes","Gurukul Institute","Rise Academy","NextGen Tutorials"].map((a,i)=>(<div key={a} style={{fontSize:14,fontWeight:700,color:C.t3,opacity:0.7+i*0.05}}>{a}</div>))}
      </div>
    </section>
  );
}
...
function HowItWorks() {
  return (
    <section id="how-it-works" style={{padding:"100px 5%",background:`linear-gradient(180deg,${C.bg} 0%,${C.bg2} 100%)`}}><div style={{maxWidth:900,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:4}}>
        {STEPS.map((s,i)=>(<div key={s.num} style={{display:"flex",gap:24,alignItems:"flex-start",position:"relative"}}>{i<STEPS.length-1&&<div style={{position:"absolute",left:27,top:56,width:2,height:60,background:`linear-gradient(${s.color}44,transparent)`}}/>}<div style={{width:56,height:56,borderRadius:"50%",flexShrink:0,background:`${s.color}18`,border:`2px solid ${s.color}44`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>{s.icon}</div><div style={{paddingBottom:52}}><div style={{fontSize:11,color:s.color,fontWeight:700,marginBottom:4,letterSpacing:"0.1em"}}>STEP {s.num}</div><h3 style={{fontSize:18,fontWeight:700,color:C.t1,marginBottom:8}}>{s.title}</h3><p style={{fontSize:15,color:C.t2,lineHeight:1.7}}>{s.desc}</p></div></div>))}
      </div>
    </div></section>
  );
}
...
function Pricing({ onGetStarted }) {
  ...
  return (
    <section id="pricing" style={{padding:"100px 5%"}}><div style={{maxWidth:1100,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:14,marginBottom:48}}>
        ...
        <div onClick={()=>setAnnual(!annual)} style={{width:48,height:26,borderRadius:13,background:annual?C.acc:C.border2,cursor:"pointer",position:"relative",transition:"background 0.2s"}}><div style={{position:"absolute",top:3,left:annual?25:3,width:20,height:20,borderRadius:"50%",background:"#fff",transition:"left 0.2s",boxShadow:"0 1px 4px rgba(0,0,0,0.3)"}} /></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:20,alignItems:"start"}}>
        {PLANS.map(p=>(
          <div key={p.id} style={{background:p.popular?`linear-gradient(135deg,${C.acc}14,${C.pur}0a)`:C.bg2,border:`2px solid ${p.popular?C.acc:C.border}`,borderRadius:20,padding:32,position:"relative",transform:p.popular?"scale(1.03)":"none",boxShadow:p.popular?`0 24px 80px ${C.acc}22`:"none"}}>
            ...
            <div style={{marginBottom:12}}><span style={{fontSize:40,fontWeight:900,color:C.t1}}>₹{Math.round(p.price*(annual?0.8:1))}</span><span style={{fontSize:14,color:C.t3}}>/{annual?"mo (billed yearly)":"month"}</span></div>
            <p style={{fontSize:14,color:C.t2,marginBottom:24,lineHeight:1.6}}>{p.desc}</p>
            {p.id==="enterprise"
              ?<a href={`mailto:${OWNER_EMAIL}?subject=Enterprise Plan Inquiry`} style={{display:"block",width:"100%",padding:"12px",borderRadius:10,fontWeight:700,fontSize:14,cursor:"pointer",border:"none",marginBottom:24,background:C.bg3,color:C.t1,textAlign:"center",textDecoration:"none",boxSizing:"border-box"}}>Contact Us →</a>
...
function FounderStory() {
  return (
    <section id="about" style={{padding:"80px 5%"}}><div style={{maxWidth:800,margin:"0 auto",textAlign:"center"}}>
      ...
      <div style={{marginTop:16,display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
        <a href={`mailto:${OWNER_EMAIL}`} style={{fontSize:13,color:C.t2,textDecoration:"none",padding:"7px 16px",background:C.bg2,borderRadius:8,border:`1px solid ${C.border2}`}}>📧 {OWNER_EMAIL}</a>
        <a href={`https://wa.me/${OWNER_WHATSAPP}`} target="_blank" rel="noreferrer" style={{fontSize:13,color:C.grn,textDecoration:"none",padding:"7px 16px",background:C.bg2,borderRadius:8,border:`1px solid ${C.border2}`}}>💬 WhatsApp: {OWNER_PHONE}</a>
      </div>
    </div></section>
  );
}
...
function Security() {
  ...
  return (
    <section id="security" style={{padding:"80px 5%",background:C.bg2}}><div style={{maxWidth:1100,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:20}}>
        {items.map(x=>(<div key={x.t} style={{background:C.bg3,border:`1px solid ${C.border}`,borderRadius:14,padding:24}}><div style={{fontSize:28,marginBottom:12}}>{x.i}</div><div style={{fontSize:15,fontWeight:700,color:C.t1,marginBottom:8}}>{x.t}</div><div style={{fontSize:13,color:C.t2,lineHeight:1.7}}>{x.d}</div></div>))}
      </div>
    </div></section>
  );
}
...
function FAQ() {
  ...
  return (
    <section id="faq" style={{padding:"100px 5%"}}><div style={{maxWidth:720,margin:"0 auto"}}>
      <SecHeading label="FAQ" title="Frequently Asked Questions." sub="Everything you need to know before getting started." />
      {FAQS.map((f,i)=>(<div key={i} onClick={()=>setOpen(open===i?null:i)} style={{borderBottom:`1px solid ${C.border}`,cursor:"pointer"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 0"}}><span style={{fontSize:15,fontWeight:600,color:C.t1}}>{f.q}</span><span style={{color:C.acc,fontSize:20,fontWeight:300,transform:open===i?"rotate(45deg)":"none",transition:"transform 0.2s",lineHeight:1}}>+</span></div>{open===i&&<div style={{fontSize:14,color:C.t2,lineHeight:1.8,paddingBottom:18}}>{f.a}</div>}</div>))}
    </div></section>
  );
}
...
function FinalCTA({ onGetStarted }) {
  return (
    <section style={{padding:"100px 5%",background:`radial-gradient(ellipse 70% 60% at 50% 50%,${C.acc}14 0%,transparent 70%)`,textAlign:"center"}}><div style={{maxWidth:680,margin:"0 auto"}}>
...
function Footer() {
  ...
  return (
    <footer style={{padding:"40px 5% 32px",borderTop:`1px solid ${C.border}`,background:C.bg}}><div style={{maxWidth:1100,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:32,marginBottom:40}}>
        <div style={{maxWidth:260}}>
          ...
          <div style={{marginTop:12,fontSize:12,color:C.t3,lineHeight:1.8}}>
            <div>📧 {OWNER_EMAIL}</div>
            <div>📞 {OWNER_PHONE}</div>
          </div>
        </div>
